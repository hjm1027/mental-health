...
func GetCourseFavoriteCollection(userId, limit, page uint32) ([]*model.CourseInfoResponse, error) {
	var response []*model.CourseInfoResponse

	records, err := model.GetCourseFavoriteCollectionsByUserId(userId, limit, page)
	if err != nil {
		log.Error("GetCourseFavoriteCollectionsByUserId get records error", err)
		return nil, err
	}
